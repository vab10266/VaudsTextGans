...
from utils.text_process import load_test_dict, text_process, write_tokens, tensor_to_tokens
import torch
import time
...
def program_config(parser):
    ...
    parser.add_argument('--gen_pretrain', default=1, type=int)
...
if __name__ == '__main__':
    ...
    print("init")
    inst.gen.load_state_dict(torch.load("pretrain\\amazon\\gen_MLE_00010.pt"))
    for i in range(1):
        num_examples = 200
        for label_i in range(10):
            save_sample_path = cfg.save_samples_root + 'samples_c{}_{}_0.txt'.format(label_i, num_examples)
            print(save_sample_path)
            t0 = time.time()
            samples = inst.gen.sample(num_examples, num_examples, label_i=label_i)
            tensor = tensor_to_tokens(samples, inst.idx2word_dict)
            t = time.time() - t0
            print(t)
            write_tokens(save_sample_path, tensor)
